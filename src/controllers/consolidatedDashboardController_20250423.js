...
exports.consolidatedDashboard = async (req, res) => {
    try {
        const { AccessToken, filter_data } = req.body;
        let report_data = {};
        if (AccessToken != null) {
                    ...
                    if (filter_data) {
                        ...
                        if (user_type === 6) {
                            ...
                            if (CVCA_qry_data.length > 0) {
                                ...
                                let transporter = transporter_ids.map(String);
                        ...
                        if (user_type === 6 && final_data.length > 0) {
                            ...
                            // Travel States Card
                            const [state_name] = await db.promise().query('SELECT id, code FROM state WHERE status=1');
                            ...
                            const table_UPC = "vehicle_utlization";
                            const condition_UPC = { status: 1, type_id: parseInt(trns_id) };
                            const fields_UPC = { projection: { _id: 0 } };
...
exports.consolidatedSummarydDashboard = async (req, res) => {
    try {
        const { AccessToken, filter_data, sub_type } = req.body;
        let report_data = {};
        if (AccessToken != null) {
            const user_info = await tokenWeb.isAuthorizedAccessWeb(AccessToken);
                ...
                if (result.length > 0) {
                    ...
                    if (filter_data) {
                        ...
                        if (user_type === 6) {
                            ...
                            const LRA_qry = `SELECT * FROM logistic_role_assignment Status=1 AND WHERE user_id=${user_id}`;
                            const [LRA_qry_data] = await db.promise().query(LRA_qry);
                        ...
                        if(final_data){
                            if (sub_type === 'vehicle_status') {
                                ...
                                const table_UData = 'vehicle_utlization';
                                const condition_UData = { status: 1, type_id: parseInt(transporter[0]) };
                                const fields_UData = { projection: { _id: 0 } };
