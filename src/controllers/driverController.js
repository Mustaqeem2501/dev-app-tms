        ...
        if (AccessToken != null) {
            ...
            if (user_info && user_info.Status === 2) {
                ...
                if (result.length > 0) {
                    ...
                    // Fetch Document Type
                    const [document_result] = await db.promise().query("SELECT id, name FROM document_types WHERE status=1 AND category='driver'");
                    // res.send(document_result);
                    ...
                    //Fetch Transporters
                    if(user_type != 10){
                        let customer_query_data;
                        if (group_type === 32) {
                            // console.log('user_type');process.exit(0);
                            [customer_query_data] = await db.promise().query(
                                // `SELECT * FROM cv_customer_assignment WHERE status=1 AND user_id=${user_id} AND type_detail='Transporter'`
                                `SELECT cvca.type_detail_id AS transporter_id t.name AS transporter_name 
                                FROM cv_customer_assignment AS cvca 
                                LEFT JOIN transporters AS t ON cvca.type_detail_id = t.id 
                                WHERE cvca.status=1 AND cvca.user_id=${user_id} AND cvca.type_detail='Transporter'`
                            );
                        ...
                        customer_query_data.forEach(value => {
                    ...
                    data.documnet_type = document_result;
...
exports.driverDashboard = async (req, res) => {
    ...
    try {
        ...
        if (AccessToken == null) {
            ...
            }); // Return early if AccessToken is missing
        }
            ...
            if (response.length > 0) {
                    ...
                    if(user_type != 10){
                        ...
                        if (group_type === 32) {
                            ...
                            if(group_type === 3){
                                ...
                                let user_ids = U_qry_data.map(item => item.id).join(',');
                                ...
                                if (!LRA_TId_qry_data.length) {
                                    return res.status(200).json({ Status: 'Fail', Message: 'Transporter id not available' });
                                }
                ...
                if(transporter){
                    ...
                    if (!LRA_UId_qry_data.length) {
                        return res.status(200).json({ Status: 'Fail', Message: 'User id not available' });
                    }
                ...
                if(driverIds.length<500){
                    ...
                    [Driver_qry_data] = await db.promise().query(`SELECT d.id AS driver_id, d.name AS name, d.name_hn AS name_hn, d.gender AS gender, DATE_FORMAT(d.birth_date, '%d-%m-%Y') AS birth_date, d.mob_no AS mob_no, d.alternate_mob_no AS alternate_mob_no, d.email AS email, d.state_id AS state_id, s.name AS state_name, d.city_id AS city_id, c.name AS city_name, d.address AS address, d.pincode AS pincode, d.class_of_vehicle AS class_of_vehicle, DATE_FORMAT(d.driving_start_date, '%Y-%m-%d') AS driving_start_date, d.is_authenticated AS is_authenticated, d.status AS status FROM drivers AS d LEFT JOIN state AS s ON d.state_id = s.id LEFT JOIN city AS c ON d.city_id = c.id WHERE d.status IN(1,2) AND d.id IN(${driverIds})`);
                    ...
                    for (const chunk of driverChunks) {
                        ...
                        drivr_qry = `
                            SELECT 
                                ...
                                d.gender AS gender,
                                DATE_FORMAT(d.birth_date, '%d-%m-%Y') AS birth_date,
                                d.mob_no AS mob_no,
                                ...
                                DATE_FORMAT(d.driving_start_date, '%Y-%m-%d') AS driving_start_date,                                
                            ...
                            LEFT JOIN 
                                city AS c ON d.city_id = c.id 
                            WHERE 
                                d.status IN(1,2) AND d.id IN(${DriverIdString})`
                        ;
                ...
                const driver_data = Driver_qry_data.map((driver, index) => {
                    ...
                    // TmpDriverData.BirthDate = (driver.birth_date && driver.birth_date !== "0000-00-00") ? formatDate(driver.birth_date) : "";
                    TmpDriverData.BirthDate = (driver.birth_date && driver.birth_date !== "0000-00-00") ? driver.birth_date : "";
                    ...
                    TmpDriverData.Status = driver.status === 1 ? "Active" : driver.status === 2 ? "Deactive" : driver.status === 3 ? "Blacklist" : "Unknown";
                    TmpDriverData.IsAuthenticated = driver.is_authenticated === 1 ? "Authenticated" : "Non-Authenticated";


                    // Drivers Enrollment Count For Tile
                    if (driver.is_authenticated === 1 && driver.status === 1) driverEnrolmentCount.authenticated_drivers++;
                    if (driver.is_authenticated === 0 && driver.status === 1) driverEnrolmentCount.enrolled_drivers++;
...
exports.driverAdd2026 = async(req, res) =>{
    const dateToday= moment().tz('Asia/Calcutta').format("YYYY-MM-DD");
    let user_info ={};
    try{
        let response = {};
        // console.log('AccessToken');process.exit(0);
        const {AccessToken,driver_data, DeveloperOptionId, DeveloperOption} = req.body;
        ...
        if(AccessToken !== null){
            ...
            if (user_info && user_info.Status === 2) {
                ...
                if (response.length > 0) {
                    ...
                    try{
                        const [insertedResult] = await db.promise().query(sql, values);
                        insertResult = insertedResult;
                    ...
                    //     return res.status(501).json({ Status: "fail", Message: insertResult.Message });
                    // }
            ...
            res.status(501).json({ Status: "Fail", Message: "payload missing"});
        }
    }catch(error){
            ...
            if (user_info && user_info.Status === 2) {
                ...
                if (response.length > 0) {
                    ...
                    const {
                        ...
                        birth_date: birth_date,
                        mob_no: mob_no,
                        is_mob_no_validate: is_mob_no_validate,
                        alternate_mob_no: alternate_mob_no,
                        email: email,
                    ...
                    const sql = `
                        UPDATE drivers SET 
                            name = ?, gender = ?, birth_date = ?, mob_no = ?, is_mob_no_validate = ?, alternate_mob_no = ?, email = ?, country_id = ?, state_id = ?, city_id = ?, address = ?, address2 = ?, pincode = ?, class_of_vehicle = ?, driving_start_date = ?, is_authenticated = ?, edit_id = ?, edit_date = ? WHERE id = ?
                    `;                    
                    
                    const values = [
                        name, gender, birth_date, mob_no, is_mob_no_validate, alternate_mob_no, sanitizedEmail, country_id, state_id, city_id, address, address2, pincode, formattedClassOfVehicle, formattedDrivingStartDate, is_authenticated, edit_id, edit_date, driverId
                    ];     
...
exports.driverAction = async(req, res) =>{
    ...
    try{
        let response = {};
        const { AccessToken, driver_id, action, DeveloperOptionId, DeveloperOption } = req.body;
                ...
                if (response.length > 0) {
                    ...
                    for (let id of idsArray) {
                        ...
                        if(action == 'verify'){
                            ...
                            let [resVehicle] = await db.promise().query(sqlDriver, valDriver);
                        }else{
                            let sqlDriver = `
                            ...
                            let [resVehicle] = await db.promise().query(sqlDriver, valDriver);
                            // res.send({'resVehicle':resVehicle,'valDriver':valDriver,'status':status,'editId':editId,'editDate':editDate});
                            const sqlDoc = `
...
exports.getLocationFromPincodeFilter = async(req,res)=>{
    try{
        let resData = {};      
        resData.Status="fail";
        
        const {DeveloperOption, DeveloperOptionId, AccessToken, searchQuery} = req.body;

        if(!AccessToken && (AccessToken == null || AccessToken == '')){
            resData.Message="AccessToken payload is missing.";
        ...
        if (!isAccessTokenValid || isAccessTokenValid.Status === 'fail') {
            ...
            return res.status(401).json(isAccessTokenValid || resData);
        }
        ...
        let group_id = isAccessTokenValid.group_id;
        
        let [pincodeData] = await db.promise().query(`SELECT * FROM pincode_city WHERE status = ? AND pin_code LIKE ?`,
            [1, `%${searchQuery}%`]
        );
        ...
        let filterData = {'states': states, 'cities': cities, 'country':{'id':1, 'name':'India'}};
        ...
        if(pincodeData.length > 0){
            ...
            resData.data = branches;
            return res.status(200).json(resData);
        }

    }catch(error){
        resData.Status = "error";
        resData.Message = "Internal Server Error while fetching pincode location data.";
        resData.Error = error.message;
        return res.status(500).json(resData); // ✅ 500 = Server error
    }
};

exports.driverMasterFilter = async (req, res) => {
    let resData = {
        Status: "fail",
        Message: "",
        data: null
    ...
    try {
        const { DeveloperOption, DeveloperOptionId, AccessToken } = req.body;
        ...
        const { id: userId, user_type, group_type, group_id } = auth;
        ...
        if (user_type !== 10) {
            ...
            if (group_type === 32) {
                query = `
                    SELECT 
                        cvca.type_detail_id AS id,
                        COALESCE(t.name, '') AS name
                    FROM cv_customer_assignment AS cvca
                    LEFT JOIN transporters AS t 
                        ON cvca.type_detail_id = t.id
                    WHERE cvca.status = 1 
                    AND cvca.user_id = ?
                    AND cvca.type_detail = 'Transporter'
                    AND t.status = 1
                `;
                params = [userId];
            } else {
                query = `
                    SELECT 
                        tca.transporter_id AS id,
                        COALESCE(t.name, '') AS name
                    FROM transporter_customer_assiginment AS tca
                    LEFT JOIN transporters AS t 
                        ON tca.transporter_id = t.id
                    WHERE tca.status = 1 
                    AND tca.customer_group_id = ?
                    AND t.status = 1
                `;
                params = [group_id];
            }
            ...
            customer_filter = rows;
        }

        // ✅ Fetch driver permissions
        const [kycPermission] = await db.promise().query(
            `SELECT JSON_OBJECTAGG(kpm.name, ukp.is_allowed) AS permissions
        ...
        if ( kycPermission.length > 0 && kycPermission[0].permissions ) {
            const parsed = JSON.parse(kycPermission[0].permissions);
            
            driver_permissions = Object.entries(parsed).map(([key, value]) => ({
                [key]: value
            }));
        }
        ...
        console.error("driverMasterFilter Error:", error);
...
exports.driverAddNew = async (req, res) => {
    // const resData = {
    //     Status: "fail",
    //     Message: "",
    //     data: null
    ...
    try {
        const { DeveloperOption, DeveloperOptionId, AccessToken, driver_data } = req.body;
        // res.send(driver_data);process.exit(0);
        // ✅ AccessToken validation
        if (!AccessToken) {
            return res.status(400).json({ Status: "fail", Message: "AccessToken payload is missing." });
        }

        const auth = await checkAccessToken(AccessToken, DeveloperOptionId, DeveloperOption);

        if (!auth || auth.Status === "fail") {
            return res.status(401).json({ Status: "fail", Message: "Invalid access token." });
        }

        const { id: userId, group_type, group_id } = auth;

        if (group_type === 32) {
            return res.status(403).json({ Status: "fail", Message: "Permission denied to add driver." });
        }

        // ✅ Parse JSON safely
        let inputData;
        try {
            inputData = typeof driver_data === "string" ? JSON.parse(driver_data) : driver_data;
        } catch (err) {
            return res.status(400).json({ Status: "fail", Message: "Invalid driver_data JSON" });
        }
        ...
        // ✅ Basic validation
        if (!name || !mob_no) {
            return res.status(400).json({ Status: "fail", Message: "Name and mobile number are required." });
        }

        const createDate = moment().tz('Asia/Calcutta').format('YYYY-MM-DD HH:mm:ss');

        const formattedClassOfVehicle = Array.isArray(class_of_vehicle)
            ? class_of_vehicle.join(',')
            : null;

        const formattedDrivingStartDate = driving_start_date
            ? moment(driving_start_date).format('YYYY-MM-DD')
            : null;
        ...
        const [driverResult] = await connection.query(
            `INSERT INTO drivers (
                name, gender, birth_date, mob_no, is_mob_no_validate, alternate_mob_no, email,
                country_id, state_id, city_id, address, address2, pincode,
                class_of_vehicle, driving_start_date,
                is_authenticated, status, create_id, create_date
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                name,
                gender || null,
                ...
                address2,
                pincode || null,
                formattedClassOfVehicle,
                formattedDrivingStartDate,
                0,
                1,
                userId,
                createDate
        ...
        // ✅ Resolve userId for mapping
        let mappingUserId = userId;

        if (TransporterId) {
            const [rows] = await connection.query(
                `SELECT user_id FROM logistic_role_assignment WHERE type_detail_id = ? AND status = 1`,
                [TransporterId]
            );

            if (rows.length > 0) {
                mappingUserId = rows[0].user_id;
            }
        }

        // ✅ Insert mapping
        const [mappingResult] = await connection.query(
            `INSERT INTO driver_group_user_mapping
            (group_id, user_id, driver_id, status, create_id, create_date)
            VALUES (?, ?, ?, ?, ?, ?)`,
            [group_id, mappingUserId, driverId, 1, userId, createDate]
        );
        ...
        console.error("driverAdd Error:", error);
...
exports.driverEditDetails2026 = async (req, res) => {
    //const connection = await db.promise().getConnection();

    try {
        const { DeveloperOption, DeveloperOptionId, AccessToken, id } = req.body;
        // res.send(driver_data);process.exit(0);
        // ✅ AccessToken validation
        if (!AccessToken) {
            return res.status(400).json({ Status: "fail", Message: "AccessToken payload is missing." });
        ...
        let [resultDriver] = await db.promise().query(`SELECT 
                name, gender, DATE_FORMAT(birth_date, '%Y-%m-%d') AS birth_date, mob_no, is_mob_no_validate,
                
                IFNULL(alternate_mob_no, '') AS alternate_mob_no,
                IFNULL(email, '') AS email,

                country_id,
                CAST(state_id AS CHAR) AS state_id,
                CAST(city_id AS CHAR) AS city_id,
                ...
                YEAR(driving_start_date) AS driving_start_date,
                is_authenticated

            FROM drivers 
            WHERE status = ? AND id = ?`,[1, id]
        );

        let driver = resultDriver[0];
        let finalResponse = [];
        if (driver) {
            // class_of_vehicle: "LMV,HMV,MGV" → array of objects
            let vehicleArray = [];

            if (driver.class_of_vehicle) {
                vehicleArray = driver.class_of_vehicle.split(',').map(v => ({
                    id: v,
                    name: v
                }));
            }
            ...
            finalResponse = {
                ...
                mob_no: driver.mob_no,
                is_mob_no_validate: driver.is_mob_no_validate,
                alternate_mob_no: driver.alternate_mob_no,
                email: driver.email,
                ...
                class_of_vehicle: vehicleArray,

                driving_start_date: driver.driving_start_date.toString(),
                is_authenticated: driver.is_authenticated
            };
        }
        // res.send(finalResponse);process.exit(0);    
        // if (Is_all_data === 'yes') {
        //     [Document_qry_data] = await db.promise().query(`SELECT doct.name AS document_name, doc.id, doc.driver_id, doc.doc_type_id, doc.doc_no, DATE_FORMAT(doc.issue_date, '%Y-%m-%d') AS issue_date, DATE_FORMAT(doc.expiry_date, '%Y-%m-%d') AS expiry_date, doc.file_path, doc.status FROM documents AS doc LEFT JOIN document_types AS doct ON doct.id = doc.doc_type_id WHERE doc.status IN(1,2) AND doct.status = 1 AND doc.driver_id IN(${driverIds}) AND doc.create_date BETWEEN '${fromDate}' AND '${toDate}'`);
        // } else {
        //     [Document_qry_data] = await db.promise().query(`SELECT doct.name AS document_name, doc.id, doc.driver_id, doc.doc_type_id, doc.doc_no, DATE_FORMAT(doc.issue_date, '%Y-%m-%d') AS issue_date, DATE_FORMAT(doc.expiry_date, '%Y-%m-%d') AS expiry_date, doc.file_path, doc.status FROM documents AS doc LEFT JOIN document_types AS doct ON doct.id = doc.doc_type_id WHERE doc.status IN(1,2) AND doct.status = 1 AND doc.driver_id IN(${driverIds})`);
...
exports.driverEditDetails = async (req, res) => {
    //const connection = await db.promise().getConnection();
    ...
    try {
        ...
        let [driverResult] = await db.promise().query(`SELECT 
                d.id,
                d.name,
                JSON_OBJECT('id', d.gender, 'name', d.gender) AS gender,
                
                DATE_FORMAT(d.birth_date, '%Y-%m-%d') AS birth_date,
                d.mob_no,
                d.is_mob_no_validate,
                
                IFNULL(d.alternate_mob_no, '') AS alternate_mob_no,
                IFNULL(d.email, '') AS email,

                d.country_id,
                CAST(d.state_id AS CHAR) AS state_id,
                CAST(d.city_id AS CHAR) AS city_id,

                d.address,
                d.address2,
                CAST(d.pincode AS CHAR) AS pincode,

                d.class_of_vehicle,
                YEAR(d.driving_start_date) AS driving_start_date,
                d.is_authenticated

            FROM drivers d
            WHERE d.status = ? AND d.id = ?`,[1, id]
        );
        if (driverResult.length > 0) {
            driverResult = driverResult.map(d => ({
                ...d,
                gender: typeof d.gender === "string" ? JSON.parse(d.gender) : d.gender
            }));
        }
        let [documents] = await db.promise().query(`
                SELECT 
                    ...
                    doc.doc_type_id,
                    doc.doc_no,
                    DATE_FORMAT(doc.issue_date, '%Y-%m-%d') AS issue_date,
                    DATE_FORMAT(doc.expiry_date, '%Y-%m-%d') AS expiry_date,
                    doc.file_path,
                    doc.status
                FROM documents doc
                INNER JOIN (
        ...
        // res.send(documents);process.exit(0);
        ...
        // 🔹 Documents transform
        let documentList = documents.map(doc => ({
            id: doc.id,
...
async function checkAccessToken(AccessToken, DeveloperOptionId, DeveloperOption) {
    ...
    [resultUser] = await db.promise().query(
        "SELECT `id`, `user_type`, `name`, `group_type`, `group_id`, `username`, `email` FROM user WHERE `id`=? AND `status`=?",
        [user_id, 1]
    );
