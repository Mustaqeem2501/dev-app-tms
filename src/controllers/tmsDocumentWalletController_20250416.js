...
exports.documentWalletMaster = async (req, res) => {
    ...
    try {
        const { AccessToken, DeveloperOptionId, DeveloperOption } = req.body;
        ...
        if (AccessToken != null) {
            ...
            return res.status(501).json(resData); // Send response and stop further execution
        }
    } catch (error) {
...
exports.documentWallet = async (req, res) => {
    ...
    try {
        ...
        let { AccessToken, date_type, from_date, to_date, status_type_id, doc_type_id, vehicle_id, vertical_id, agent_id, transporter_id, DeveloperOptionId, DeveloperOption} = req.body;
        if (AccessToken != null) {
            if(DeveloperOptionId && DeveloperOption && DeveloperOption=="dev0.01_"+dateToday) {
                user_info.Status=1; 
                user_info.AccountId=DeveloperOptionId;
...
exports.documentAdd = async (req, res) => {
    ...
    try {
        ...
        let { AccessToken, GroupId, Category, DriverId, VehicleId, DocumentTypeId, DocumentTypeName, DocumentNo, IssueDate, ExpiryDate, Remark, DeveloperOptionId, DeveloperOption} = body;
        ...
        if (AccessToken != null) {
            ...
            if (user_info && user_info.Status === 2) {
                ...
                if(result.length>0) {                    
                    ...
                    let docNumValidation = await validateDocumentNo('OldDocumentNo', DocumentNo, 'add');
                    if (docNumValidation === 'Yes') {
                        response.Status = "fail";
                        response.Message = "Document Number is already exist in database. Please check once";
                        return res.status(200).json(response);
                    }
                    // res.send('AA');
...
exports.documentEdit = async (req, res) => {
    ...
    try {
        ...
        let { AccessToken, DocumentId, DocumentNo, DocumentTypeId, IssueDate, ExpiryDate, Remark, DeveloperOptionId, DeveloperOption} = req.body;
        ...
        if (AccessToken != null) {
            ...
            if (user_info && user_info.Status === 2) {
                ...
                if(result.length>0) {                    
                    ...
                    let oldDocumentNo = await get_document_no(DocumentId);
                    if(oldDocumentNo !== DocumentNo){
                        
                        let docNumValidation = await validateDocumentNo(oldDocumentNo, DocumentNo, 'edit');
                        // res.send(docNumValidation);
                        if (docNumValidation === 'Yes') {
                            response.Status = "fail";
                            response.Message = "Document Number is already exist in database. Please check once";
                            return res.status(200).json(response);
...
exports.documentBulkUpload = async (req, res) => {
    try {
        ...
        const user_info = await tokenWeb.isAuthorizedAccessWeb(body.AccessToken);
        // res.send(user_info);
        if (!user_info) {
            return res.status(401).json({ message: "Unauthorized access." });
        }
        ...
        if (!data || data.length === 0) {
            return res.status(400).json({ Status: "fail", message: "No data found in excel." });
        }
        
        const document_type_bin = await getDocumentTypeId();
        const document_nos_bin = await getDocumentNoList();
        ...
        for (const row of data) {
            let clean_document_no = '';
            let string_document_no = '';
            ...
            const rawDoc = row.Document_Number;
            
            if(rawDoc){
                string_document_no = String(rawDoc);
                clean_document_no = (string_document_no || '').trim();
                // if (typeof string_document_no === 'string') {
                //     clean_document_no = Number(string_document_no.trim());
                // } else if (typeof string_document_no === 'number') {
                //     clean_document_no = string_document_no;
                // }
                // res.send(clean_document_no);
            }
            

            
            const document_no = clean_document_no || '';
            // res.send(document_no);
            ...
            if (!category) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: DocumentCategory is missing.`);
                continue;
            }

            if (!['driver', 'vehicle'].includes(category)) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: DocumentCategory is invalid.`);
                continue;
            }

            if (category === 'vehicle' && !vehicle_driver) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: VehicleNo is missing.`);
                continue;
            }

            if (category === 'driver' && !vehicle_driver) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: DriverName is missing.`);
                continue;
            }

            if (category === 'driver' && !driver_mobile) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: DriverMobileNo is missing.`);
                continue;
            }

            if (!document_type) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: DocumentType is missing.`);
                continue;
            }
            ...
            if (!document_type_id) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: DocumentType: ${document_type} is invalid.`);
                continue;
            }

            if (['bank_passbook', 'photograph'].includes(document_type_key)) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: DocumentType: ${document_type} is not allowed for bulk upload.`);
                continue;
            }

            if (!document_no) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: DocumentNo is missing.`);
                continue;
            }

            if (!['aadhaar_card', 'pan_card', 'bank_passbook', 'photograph'].includes(document_type_key) && !issue_date) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: IssueDate is missing.`);
                continue;
            }

            if (!['aadhaar_card', 'pan_card', 'bank_passbook', 'photograph'].includes(document_type_key) && !expiry_date) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: ExpiryDate is missing.`);
                continue;
            }

            if (issue_date && expiry_date && new Date(issue_date) > new Date(expiry_date)) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: Issue/Expiry date is invalid.`);
                continue;
            }
            ...
            if (expiry_date) {
                ...
                if (days < 30) {
                    errors.push(`Row: ${data.indexOf(row) + 1}, Message: Invalid document validity.`);
                    continue;
                }
            }
            ...
            if (category === "driver") {
                ...
                vehicle_no = cleanVehicleNo(vehicle_no);
            }
            ...
            if (category === "vehicle" && vehicle_no) {
                if (vehiclesList.hasOwnProperty(vehicle_no)) {
                    ...
                    errors.push(`Row: ${data.indexOf(row) + 1}, Message: Vehicle Number does not exist in database.`);
                    continue;
                }
            }
            ...
            if (category === "driver" && driver_mobile) {
                if (driversList.hasOwnProperty(driver_mobile)) {
                    driver_id = driversList[driver_mobile];
                } else {
                    errors.push(`Row: ${data.indexOf(row) + 1}, Message: DriverId does not exist in database.`);
                    continue;
                }
            }

            const create_date = moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:mm:ss');
            // res.send({"issue_date":formatDate(issue_date)});
            const document = {
                group_id: group_id,
                user_id: user_id,
            ...
            // res.send(document);
            const insertResult = await addDocumentEntry(document);
            // res.send(insertResult);
            if (!insertResult.document_status) {
                errors.push(`Row: ${data.indexOf(row) + 1}, Message: Document data could not be saved.`);
            } else {
                insertedRowsCount++;
                insertedIds.push({
                    vehicle_driver: vehicle_driver,
                    document_id: insertResult.document_id,
                    reminder_id: insertResult.reminder_id
                });
            }
        }
        ...
        return res.status(200).json({
            ...
            Errors: errors,
            result: {
                insert_rows_count: insertedRowsCount,
...
async function addDocumentEntry(document) {
    const file_path = '';
    const thumb_file_path = '';

    const sql = `
        INSERT INTO documents 
        (group_id, user_id, driver_id, vehicle_id, doc_type_id, doc_no, issue_date, expiry_date, file_path, thumb_path, remark, status, create_id, create_date) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const params = [
        document.group_id,
    ...
    try {
        ...
        if (document.expiry_date && result.insertId) {
            try {
                const sql2 = `
                    INSERT INTO v2_reminders 
                    (group_id, user_id, rem_category_id, vehicle_id, driver_id, document_id, rem_type_id, rem_type_name, due_date, status, create_id, create_date) 
                ...
                const params2 = [
                    ...
                    document.vehicle_id,
                    document.driver_id,
                    result.insertId, // Use the inserted document ID
                    2, // rem_type_id (Document renewal)
                    document.doc_type_name || 'Document', // Default to 'Document' if not provided
                    document.expiry_date,
                    document.status,
                    document.create_id,
                    document.create_date
                ...
                const [result2] = await db.promise().query(sql2, params2);
                reminder_id = result2.insertId;
            } catch (error) {
                console.error("Reminder Insertion Error:", error);
                // Continue even if reminder insertion fails
            }
        }

        return {
            document_status: result.affectedRows > 0,
            document_id: result.insertId,
            reminder_status: reminder_id !== null,
...
// async function validateDocumentNo(DocumentNo) {
    
//     let add_query = action==='add' ? ' = ?' : ' != ?';
//     // const sql = `SELECT * FROM documents WHERE status=1 AND doc_no = ?`;
//     const sql = `SELECT * FROM documents WHERE status=1 AND doc_no = ${add_query}`;
//     const [result] = await db.promise().query(sql, [DocumentNo]);
    
//     let isExist = result.length > 0 ? 'Yes' : 'No';
//     return isExist;
// }

async function validateDocumentNo(oldDocumentNo, DocumentNo, action) {
    // const operator = action === 'add' ? '=' : '!=';
    // const sql = `SELECT * FROM documents WHERE status=1 AND doc_no ${operator} ?`;
    ...
    if (action === 'add') {
        sql = `SELECT * FROM documents WHERE status = 1 AND doc_no = ?`;
        values = [DocumentNo];
    } else if (action === 'edit') {
        // let oldDocumentNo = await get_document_no(DocumentId);
        sql = `SELECT * FROM documents WHERE status = 1 AND doc_no = ? AND doc_no != ?`;
        values = [DocumentNo, oldDocumentNo];
    }
...
async function get_document_no(doc_id) {
    let doc_no_old = '';
    const [docResult] = await db.promise().query(
        `SELECT doc_no FROM documents WHERE status = 1 AND id = ?`, 
        [doc_id]
    );
    
    if (docResult.length > 0) {
        doc_no_old = docResult[0].doc_no;
    }
