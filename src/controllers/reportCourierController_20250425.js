        ...
        if(AccessToken!=null){
            ...
            if (user_info && user_info.Status === 2) {
                ...
                if (resultUser.length > 0) {
                    ...
                    // res.send({'userType':userType,'group_id':group_id,'user_id':user_id});
                    ...
                    if(region && transporter_id && vehicle_id){
                        ...
                        if (vIds && vIds.length > 0) {
                            ...
                            //Step_4:
                                ...
                                const [resultT] = await db.promise().query("SELECT `id`,`name`, `code` FROM transporters WHERE `status`= ? AND `id` IN(?)",[1,t_Ids]);
                                //const [resultT] = await db.promise().query("SELECT `id`,`name`, `code` FROM transporters WHERE `status`= ? AND `group_id` IN(?)",[1,group_id]);
                        ...
                        if (vIds && vIds.length > 0) {
                            ...
                            //Step_5:
                                ...
                                //console.log(Doc_data);process.exit(0);
                            ...
                            //Final Step:
                                final_array = resultVDA.map(vehicle => {
                                    ...
                                    // if(vehicle.vehicle_number === 'MH03DV3240'){
                                    //     res.send(vwhInfo.flag);
                                    // }
                                    // Add device data for each device_id (if exists)
