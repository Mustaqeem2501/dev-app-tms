            ...
            if (response.length > 0) {
                ...
                if(driverIds.length<500){
                    ...
                    for (const chunk of driverChunks) {
                        ...
                        let doc_qry = '';
                        if (Is_all_data === 'yes') {
                            doc_qry = `SELECT doct.name AS document_name, doc.id, doc.driver_id, doc.doc_type_id, doc.doc_no, doc.issue_date, doc.expiry_date, doc.file_path, doc.status 
