            ...
            if (user_info && user_info.Status === 2) {
                ...
                if (result.length > 0) {
                    ...
                    if (filter_data) {
                        ...
                        if (user_type === 6) {
                            ...
                            if (flag_of_date === 1) {
                                if (flag_of_agent === 0) {
                                    ...
                                    CVCA_qry = `SELECT type_detail, type_detail_id FROM cv_customer_assignment WHERE status=1 AND user_id=${user_id} AND DATE(create_date) <= '${to_date}'`;
                                } else {
                                    ...
                                    CVCA_qry = `SELECT id, transporter_id FROM cv_transporter_assignment WHERE status=1 AND type_id=${agent_id} AND DATE(create_date) <= '${to_date}'`;
                                }
                            ...
                            let TCA_qry_data = [];
                            if ((CVCA_qry_data.length > 0) && (flag_of_cv_customer === 1)) {
                            ...
                            // return res.send((CVCA_qry_data.length > 0));
                            if ((CVCA_qry_data.length > 0) || (TCA_qry_data.length > 0)){
                                // return res.send(TCA_qry_data);
                                ...
                                if (transporter.length > 0) {
                                    ...
                                    return res.status(200).json({ Status: 'Fail', Message: 'Data not found.', Remark: 'Data not available with these filters' });
                        ...
                        // return res.send(transporter.length);
                        ...
                        if (user_type === 6 && final_data.length > 0) {
                            ...
                            if (Object.keys(Trnsp_ids).length > 0) {
                                ...
                                const riskdata = await processRiskDataSummary(transporter, lastMonth, lastMonthEnd);
                                risk_vehicle_status = riskdata.summaryCount;
                                // let riskDataCount = riskdata.summaryCount;
                                // res.send(riskDataCount.length);exit(0);
                                // if(riskDataCount.length > 0){
                                //     risk_vehicle_status = riskdata.summaryCount;
                                // }else{
                                ...
                                //     // res.send(riskdata.length);exit(0);
                                //     if(riskdata.length > 0){
                                //         let scoreData = await processRiskData(riskdata);
                                        
                                //         // res.send(scoreData.main_riskscore_summary);
                                //         risk_vehicle_status = scoreData.main_riskscore_summary;
                                //         // risk_vehicle_status = scoreData.on_ground_riskscore_summary;
                        ...
                        if (user_type === 6 && final_data.length > 0) {
                                ...
                                if (Trns_id_to_Ver_id[value.transporter_id]) {
                                    ...
                                    // if (vas_name[Trns_id_to_Ver_id[T_id]]) {
                                        ...
                                        if (vas_data[vert_name]) {
                                            // res.send({vas_data,vas_name,T_id,Trns_id_to_Ver_id});exit(0);
                            ...
                            // res.send(Trns_id_to_Ver_id[T_id]);exit(0);
                            // res.send(vas_data);exit(0);
                            ...
                            const condition = { group_id: group_id, date: { $gte: formatDate(fromDate), $lte: dateToday } };
                            // res.send(condition);process.exit(0);
                            const getFields = { projection: { _id: 0 } };
                            ...
                            // const tablerisk = "risk_information_secutrak";
                            // const conditionrisk = { status: 1, vehicle_id:{$in:vehicleIdArrayString}, date: { $gte: lastMonth, $lte: lastMonthEnd } };
                            // const getFieldsrisk = { _id: 0, date: 1, score_deduction_percent:1, vehicle_id:1 };
                            // // const getFieldsrisk = { };
                            // const riskdata = await getMongo.getBAMongoQuery(conditionrisk, { projection: getFieldsrisk }, tablerisk);
...
async function processRiskDataOLD20251108(riskdata) {
...
async function processRiskData(riskdata) {
    // ============================================================
    // Step 1️⃣: Filter - For each vehicle + date, take only one record
    // ============================================================
    const filteredDataMap = {};
    // return riskdata;
    riskdata.forEach(item => {
        const key = `${item.vehicle_id}_${item.date}`;
        const score = item.score_deduction_percent;

        if (!filteredDataMap[key]) {
            filteredDataMap[key] = item; // first record
        } else {
            // keep the one with highest score_deduction_percent
            if (score > filteredDataMap[key].score_deduction_percent) {
                filteredDataMap[key] = item;
            }
        }
    });
    
    // Convert map to array for further processing
    const filteredData = Object.values(filteredDataMap);

    // ============================================================
    // Step 2️⃣: Main Risk Score Calculation (based on all filtered data)
    // ============================================================
    const vehicleMap = {};
    filteredData.forEach(item => {
        const vid = item.vehicle_id;
        if (!vehicleMap[vid]) vehicleMap[vid] = [];
        vehicleMap[vid].push(item.score_deduction_percent);
    });

    const summary_main = { high: 0, avg: 0, low: 0 };

    for (const [vehicle_id, scores] of Object.entries(vehicleMap)) {
        const avg = scores.reduce((a, b) => a + b, 0) / scores.length;
        if (avg > 70) summary_main.high++;
        else if (avg > 30) summary_main.avg++;
        else summary_main.low++;
    }

    // ============================================================
    // Step 3️⃣: On-Ground Risk Score (only scores > 0)
    // ============================================================
    const onGroundMap = {};
    filteredData
        .filter(item => item.score_deduction_percent > 0)
        .forEach(item => {
            const vid = item.vehicle_id;
            if (!onGroundMap[vid]) onGroundMap[vid] = [];
            onGroundMap[vid].push(item.score_deduction_percent);
        });

    const summary_on_ground = { high: 0, avg: 0, low: 0 };

    for (const [vehicle_id, scores] of Object.entries(onGroundMap)) {
        const avg = scores.reduce((a, b) => a + b, 0) / scores.length;
        if (avg > 70) summary_on_ground.high++;
        else if (avg > 30) summary_on_ground.avg++;
        else summary_on_ground.low++;
    }

    // ============================================================
    // Step 4️⃣: Final Output (only summaries)
    // ============================================================
    return {
        main_riskscore_summary: summary_main,
        on_ground_riskscore_summary: summary_on_ground
    };
    
}

async function processRiskDataSummaryOLD(transporter) {
...
async function processRiskDataSummaryOLD20251108(transporter) {
  // Step 1: Vehicle mapping from MySQL
  const qryVTS = `SELECT vehicle_id FROM vehicle_transporter_assignment WHERE transporter_id IN (${transporter}) AND status=1`;
  const [resultVTS] = await db.promise().query(qryVTS);
  const vehicleIds = resultVTS.map(u => Number(u.vehicle_id));

  const [vehicleResult] = await db.promise().query(`
    SELECT id FROM vehicle 
    WHERE status=1 AND insured_id = 1 AND id IN(${vehicleIds})
  `);
  ...
  if (vehicleResult && vehicleResult.length > 0) {
    const vehicleIdArray = vehicleResult.map(v => String(v.id));

    const tablerisk = "risk_information_secutrak";
    const conditionrisk = {
      status: 1,
      vehicle_id: { $in: vehicleIdArray },
    ...
    // Step 2: Group data by vehicle_id
    const vehicleMap = {};
    riskdata.forEach(item => {
      const vId = item.vehicle_id;
      if (!vehicleMap[vId]) {
        vehicleMap[vId] = {
          ...
          running_hr: 0,
          cd_4_30_hr_count: 0,
          cd_4_30_hr_duration: 0,
          os_100_count: 0,
          os_65_to_80_count: 0,
          os_80_to_100_count: 0,
          os_100_duration: 0,
          os_65_to_80_duration: 0,
          os_80_to_100_duration: 0,
          night_driving_duration: 0
        };
      }

      const v = vehicleMap[vId];
      v.scores.push(item.final_score_percent || 0);
      v.distance_travelled += item.distance_travelled || 0;
      v.running_hr += item.running_hr || 0;
      v.cd_4_30_hr_count += item.cd_4_30_hr_count || 0;
      v.cd_4_30_hr_duration += item.cd_4_30_hr_duration || 0;
      v.os_100_count += item.os_100_count || 0;
      v.os_65_to_80_count += item.os_65_to_80_count || 0;
      v.os_80_to_100_count += item.os_80_to_100_count || 0;
      v.os_100_duration += item.os_100_duration || 0;
      v.os_65_to_80_duration += item.os_65_to_80_duration || 0;
      v.os_80_to_100_duration += item.os_80_to_100_duration || 0;
      v.night_driving_duration += item.night_driving_duration || 0;
    });

    // Step 3: Calculate summaries per vehicle
    for (const [vehicle_id, data] of Object.entries(vehicleMap)) {
      const avg_score = data.scores.length
        ? data.scores.reduce((a, b) => a + b, 0) / data.scores.length
        : 0;

      const risk_category =
        avg_score >= 80 ? "high" : avg_score >= 50 ? "avg" : "low";

      const overspeeding_incidents =
        (data.os_100_count || 0) +
        (data.os_65_to_80_count || 0) +
        (data.os_80_to_100_count || 0);

      const overspeeding_duration =
        (data.os_100_duration || 0) +
        (data.os_65_to_80_duration || 0) +
        (data.os_80_to_100_duration || 0);

      const obj = {
        vehicle_no: data.vehicle_no || null,
        risk_category,
        risk_score: Number(avg_score.toFixed(2)),
        distance_travelled: Number(data.distance_travelled.toFixed(2)) || 0,
        travel_duration_hrs: Number(data.running_hr.toFixed(2)) || 0,
        number_of_continuous_driving_incidents: data.cd_4_30_hr_count || 0,
        continuous_duration_hrs: Number(data.cd_4_30_hr_duration.toFixed(2)) || 0,
        number_of_overspeeding_incidents: overspeeding_incidents || 0,
        overspeeding_duration_hrs: Number(overspeeding_duration.toFixed(2)) || 0,
        night_driving_duration_hrs: Number(data.night_driving_duration.toFixed(2)) || 0
      };

      result.push(obj);
      summary[risk_category].push(obj);
    }
  ...
  return { summary, summaryCount, result };
}

async function processRiskDataSummaryOLD20251108172922(transporter) {
  // Step 1: Vehicle mapping from MySQL
  const qryVTS = `SELECT vehicle_id FROM vehicle_transporter_assignment WHERE transporter_id IN (${transporter}) AND status=1`;
  const [resultVTS] = await db.promise().query(qryVTS);
  const vehicleIds = resultVTS.map(u => Number(u.vehicle_id));
  ...
  const result = [];
  const summary = { high: [], avg: [], low: [] };
  const groundSummary = { high: [], avg: [], low: [] };
  ...
  if (vehicleResult && vehicleResult.length > 0) {
    ...
    const riskdata = await getMongo.getBAMongoQuery(
      conditionrisk,
      { projection: {} },
      tablerisk
    );

    // ✅ Step 2: Group by vehicle_id -> date -> imei
    const groupedData = {};

    for (const item of riskdata) {
      const vid = item.vehicle_id;
      const date = new Date(item.date).toISOString().split("T")[0];
      const imei = item.imei_no || "NA";

      if (!groupedData[vid]) groupedData[vid] = {};
      if (!groupedData[vid][date]) groupedData[vid][date] = {};
      if (!groupedData[vid][date][imei]) groupedData[vid][date][imei] = [];

      groupedData[vid][date][imei].push(item);
    }

    // ✅ Step 3: For each vehicle → pick highest score_deduction_percent per day per IMEI
    const finalVehicleMap = {};

    for (const [vehicle_id, dates] of Object.entries(groupedData)) {
      if (!finalVehicleMap[vehicle_id]) finalVehicleMap[vehicle_id] = [];

      for (const [date, imeis] of Object.entries(dates)) {
        for (const [imei, records] of Object.entries(imeis)) {
          // pick record with max score_deduction_percent
          let maxRecord = records.reduce((max, cur) =>
            (cur.score_deduction_percent || 0) > (max.score_deduction_percent || 0)
              ? cur
              : max
          );

          // if all 0, pick any (already handled by reduce)
          finalVehicleMap[vehicle_id].push(maxRecord);
        }
      }
    }

    // ✅ Step 4: Calculate risk_score and on_ground_risk_score
    for (const [vehicle_id, records] of Object.entries(finalVehicleMap)) {
      const allScores = records.map(r => r.score_deduction_percent || 0);
      const validScores = records
        .filter(r => (r.score_deduction_percent || 0) > 0)
        .map(r => r.score_deduction_percent);
      ...
      // risk category logic
      const risk_category =
        avgAll > 70 ? "high" : avgAll > 30 ? "avg" : "low";

      const ground_risk_category =
        avgValid > 70 ? "high" : avgValid > 30 ? "avg" : "low";

      const sampleRecord = records[0] || {};

      const obj = {
        vehicle_no: sampleRecord.vehicle_no || null,
        risk_category,
        risk_score: Number(avgAll.toFixed(2)),
        ground_risk_category,
        ground_risk_score: Number(avgValid.toFixed(2))
      };

      result.push(obj);
      summary[risk_category].push(obj);
      groundSummary[ground_risk_category].push(obj);
    }
  }
  ...
  const groundSummaryCount = {
    high: groundSummary.high.length,
    avg: groundSummary.avg.length,
    low: groundSummary.low.length
  };
...
async function processRiskDataSummary(transporter, lastMonth, lastMonthEnd) {
  // Step 1: Vehicle mapping from MySQL
  const qryVTS = `SELECT vehicle_id FROM vehicle_transporter_assignment WHERE transporter_id IN (${transporter}) AND status=1`;
  const [resultVTS] = await db.promise().query(qryVTS);
  const vehicleIds = resultVTS.map(u => Number(u.vehicle_id));
...
//   const lastMonthEnd = new Date(today.getFullYear(), today.getMonth(), 0);
  ...
  const result = [];
  const summary = { high: [], avg: [], low: [] };
  const onGroundSummary = { high: [], avg: [], low: [] };
    ...
    for (const [vehicle_id, dates] of Object.entries(groupedData)) {
      ...
      for (const [date, imeis] of Object.entries(dates)) {
        for (const [imei, records] of Object.entries(imeis)) {
          ...
          // if all are 0, reduce already picks one
          finalVehicleMap[vehicle_id].push(maxRecord);
        }
      }
    }

    // ✅ Step 4: Calculate per-vehicle averages + risk categories
    for (const [vehicle_id, records] of Object.entries(finalVehicleMap)) {
      const allScores = records.map(r => r.score_deduction_percent || 0);
      const validScores = records
        .filter(r => (r.score_deduction_percent || 0) > 0)
      ...
      const risk_category =
        avgAll > 70 ? "high" : avgAll > 30 ? "avg" : "low";
      const on_ground_risk_category =
        avgValid > 70 ? "high" : avgValid > 30 ? "avg" : "low";

      // aggregate additional metrics
      const distance_travelled = records.reduce((a, b) => a + (b.distance_travelled || 0), 0);
      const travel_duration_hrs = records.reduce((a, b) => a + (b.running_hr || 0), 0);
      const number_of_continuous_driving_incidents = records.reduce((a, b) => a + (b.cd_4_30_hr_count || 0), 0);
      const continuous_duration_hrs = records.reduce((a, b) => a + (b.cd_4_30_hr_duration || 0), 0);
      const os100 = records.reduce((a, b) => a + (b.os_100_count || 0), 0);
      const os65to80 = records.reduce((a, b) => a + (b.os_65_to_80_count || 0), 0);
      const os80to100 = records.reduce((a, b) => a + (b.os_80_to_100_count || 0), 0);
      const overspeeding_count = os100 + os65to80 + os80to100;

      const os100Dur = records.reduce((a, b) => a + (b.os_100_duration || 0), 0);
      const os65to80Dur = records.reduce((a, b) => a + (b.os_65_to_80_duration || 0), 0);
      const os80to100Dur = records.reduce((a, b) => a + (b.os_80_to_100_duration || 0), 0);
      const overspeeding_duration_hrs = os100Dur + os65to80Dur + os80to100Dur;

      const night_driving_duration_hrs = records.reduce((a, b) => a + (b.night_driving_duration || 0), 0);

      const vehicle_no = records[0]?.vehicle_no || null;

      const obj = {
        vehicle_no,
        risk_category,
        risk_score: Number(avgAll.toFixed(2)),
        on_ground_risk_category,
        on_ground_risk_score: Number(avgValid.toFixed(2)),

        distance_travelled: Number(distance_travelled.toFixed(2)),
        travel_duration_hrs: Number(travel_duration_hrs.toFixed(2)),
        number_of_continuous_driving_incidents,
        continuous_duration_hrs: Number(continuous_duration_hrs.toFixed(2)),
        number_of_overspeeding_incidents: overspeeding_count,
        overspeeding_duration_hrs: Number(overspeeding_duration_hrs.toFixed(2)),
        night_driving_duration_hrs: Number(night_driving_duration_hrs.toFixed(2))
      };

      result.push(obj);
      summary[risk_category].push(obj);
      onGroundSummary[on_ground_risk_category].push(obj);
    }
  }
  ...
  const onGroundSummaryCount = {
    high: onGroundSummary.high.length,
    avg: onGroundSummary.avg.length,
    low: onGroundSummary.low.length
  };
        ...
        if (AccessToken != null) {
            ...
            if (user_info && user_info.Status === 2) {
                ...
                if (result.length > 0) {
                    ...
                    const today = new Date();
                    let lastMonth = formatDate(new Date(today.getFullYear(), today.getMonth() - 1, 1));
                    let lastMonthEnd = formatDate(new Date(today.getFullYear(), today.getMonth(), 0));
                    ...
                    if (!allNull) {
                        ...
                        if(final_data){
                            ...
                            if (sub_type === 'vehicle_status') {
                                ...
                                let riskData = await processRiskDataSummary(transporter, lastMonth, lastMonthEnd);
                                // res.send(riskData);exit(0);
